...
(in-package :lambda.format.internal)
...
:COMMAS T means put a comma after every third digit."
  ...
  (cond ((and (eq radix :roman)
              (< number 4000.)
              (> number 0))
         (roman-step number 0 nil))
        ((eq radix :english)
         (english-print number))
        ((eq radix :ordinal)
         (english-ordinal-print number))
        (t
  ...
  (let ((number (if (and (numberp number) (not (floatp number)))
    ...
    (cond ((or minwidth options)
          ...
          ((numberp number)
           ...
           (format *standard-output* "~F" number))
          (t (princ number))))
  number)

