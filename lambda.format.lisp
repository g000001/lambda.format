...
(DEFVAR *FORMAT-PACKAGE* (find-package :lambda.format.internal))    ;Where format commands are interned.
...
(DEFVAR *FORMAT-PARAMS* (make-array 1024.
                                    :element-type 'character
                                    :fill-pointer 0)) ;Array for pushing parameters
...
#|(DEFRESOURCE FORMAT-PARAMS ()
  ...
  :deinitializer (setf (array-leader object 0) 0))|#   ; :DEINITIALIZER WIPE-STRUCTURE

;(DEFRESOURCE FORMAT-PARAMS ()
;  :CONSTRUCTOR (MAKE-ARRAY 10. :TYPE 'ART-Q-LIST :FILL-POINTER 0 :area format-area)
;  :INITIAL-COPIES 6 :DEINITIALIZER WIPE-STRUCTURE)


(DEFUN MAKE-FORMAT-STRING ()
  (MAKE-array 128. :element-type 'character :FILL-POINTER 0))
...
(DEFPROP FORMAT-STRING-STREAM T :IO-STREAM-P)
...
the name of a command, and remaining elements are parameters."
  ...
  (let ((*ctl-string* ctl-string)
    ...
    (let-if (typep stream '(or null string))
      ...
      (LET ((*STANDARD-OUTPUT* (COND ((NULL STREAM) (make-string-output-stream))
                                     ((STRINGP STREAM) *standard-output*)
                                     ((EQ STREAM T) *STANDARD-OUTPUT*)
                                     (T STREAM))
              #|(si:follow-all-syn-streams
        ...
        (CATCH 'FORMAT-\:^-POINT
          (CATCH 'FORMAT-^-POINT
            (TYPECASE CTL-STRING
              ...
              (ERROR (PRINC-TO-STRING CTL-STRING))
              (T (DO ((CTL-STRING CTL-STRING (CDR CTL-STRING))) ((NULL CTL-STRING))
                   (IF (STRINGP (CAR CTL-STRING))
                       ...
                       (princ *STANDARD-OUTPUT* (CAR CTL-STRING))
...
(DEFUN FORMAT-CTL-LIST (ARGS CTL-LIST &AUX (*ATSIGN-FLAG* NIL) (*COLON-FLAG* NIL))
  (FORMAT-CTL-OP (IF (GETL (CAR CTL-LIST)
                     ...
                     (INTERN (CAR CTL-LIST) *FORMAT-PACKAGE*))
                 ARGS (CDR CTL-LIST)))
...
(DEFUN FORMAT-CTL-STRING (ARGS *CTL-STRING* &AUX (*FORMAT-PARAMS* NIL))
  (UNWIND-PROTECT
      (DO ((*CTL-INDEX* 0)
        ...
        (SETQ TEM (ZL::%STRING-SEARCH-CHAR #\~ *CTL-STRING* *CTL-INDEX* *CTL-LENGTH*))
        (UNLESS (EQL TEM *CTL-INDEX*) ;Put out some literal string
          (send *STANDARD-OUTPUT* :STRING-OUT *CTL-STRING* *CTL-INDEX* TEM)
          #|(write-string *CTL-STRING* *STANDARD-OUTPUT*
                        :start *CTL-INDEX* :end TEM)|#
          (IF (NULL TEM) (RETURN))
          (SETQ *CTL-INDEX* TEM))
        ...
        (LET ((*ATSIGN-FLAG* NIL)
          ...
          (SETQ ARGS (FORMAT-CTL-OP TEM ARGS (coerce *FORMAT-PARAMS* 'list)))))
...
(DEFUN FORMAT-PARSE-COMMAND (ARGS SWALLOW-ARGS)
  (DO ((PARAM-FLAG NIL)                                ;If T, a parameter has been started in PARAM
    ...
    (COND ((CHAR<= #\0 CH #\9)   ;DIGIT-CHAR-P not loaded yet
           (SETQ TEM (- (char-code CH) (char-code #\0)))
          ...
          (T                                           ;Must be a command character
           ...
           (IF (CHAR= CH #\\)
               ...
               (LET ((I (ZL::%STRING-SEARCH-CHAR #\\ *CTL-STRING* *CTL-INDEX* *CTL-LENGTH*))
                     (*PACKAGE* *FORMAT-PACKAGE*))
                 (IF (NULL I) (FORMAT-ERROR "Unmatched \"\\\" in control string."))
                 (SETQ SYM (IF (OR (ZL::%STRING-SEARCH-CHAR #\: *CTL-STRING* *CTL-INDEX* I)
                                   (ZL::%STRING-SEARCH-CHAR #\| *CTL-STRING* *CTL-INDEX* I)
                                   )
                               (let (#|(*readtable* initial-common-lisp-readtable)|#
                                     (*readtable* (copy-readtable nil))
                                     #|(default-cons-area format-area)|#)
                                 (CL:READ-FROM-STRING *CTL-STRING* T NIL
                                                      :START *CTL-INDEX* :END I))
                               (INTERN (NSTRING-UPCASE (SUBSTRING *CTL-STRING* *CTL-INDEX* I))
                                            *FORMAT-PACKAGE*)))
                 (SETQ *CTL-INDEX* (1+ I)))
             (SETQ SYM (OR (AND (< (CHAR-INT CH) (LENGTH *FORMAT-CHAR-TABLE*))
                                (AREF *FORMAT-CHAR-TABLE* (CHAR-INT CH)))
                           (INTERN (string CH) *FORMAT-PACKAGE*))))
...
(DEFFORMAT CRLF (:NO-ARG) (IGNORE)
  (AND *ATSIGN-FLAG* (princ #\NEWLINE *STANDARD-OUTPUT*)))
...
(DEFUN FORMAT-CTL-DECIMAL (ARG PARAMS &OPTIONAL (*PRINT-BASE* 10.)      ;Also called for octal
  ...
  (COND ((AND *COLON-FLAG* (INTEGERP ARG))
        ...
        ((FIXNUMP ARG) (PRINT-FIXNUM ARG *STANDARD-OUTPUT*))
...
(DEFUN ENGLISH-PRINT-THOUSAND (N STREAM)
  (LET ((FLAG NIL)
    ...
    (WHEN (> H 0)
      (SETQ FLAG T)
      (write-string (AREF ENGLISH-SMALL (1- H)) STREAM)
      (princ #\SPACE STREAM)
      (write-string ENGLISH-100 STREAM)
      (AND (> N 0) (princ #\SPACE STREAM)))
    (COND ((= N 0))
          ((< N 20.)
           (SETQ FLAG T)
           (write-string (AREF ENGLISH-SMALL (1- N)) STREAM))
          (T
           (SETQ FLAG T)
           (write-string (AREF ENGLISH-MEDIUM (- (FLOOR N 10.) 2)) STREAM)
         ...
         (LET ((FLAG (ENGLISH-PRINT (FLOOR N 1000.) STREAM (1+ TRIAD))))
           (LET ((THIS-TRIPLET (CL:REM N 1000.)))
             (cond ((ZEROP THIS-TRIPLET)
                    FLAG)
                   (t
                    (IF FLAG (princ #\SPACE STREAM))
                    (IF (EQ FLAG 'EXPT) (write-string "plus " STREAM))
                    (ENGLISH-PRINT-THOUSAND THIS-TRIPLET STREAM)
                    (COND ((ZEROP TRIAD) T)
                          ((> TRIAD 13.)
                           (write-string " times ten to the " STREAM)
                           (ENGLISH-ORDINAL-PRINT (* 3 TRIAD))
                           (write-string " power" STREAM)
                           'EXPT)
                          (T
                           (WRITE-CHAR #\SPACE STREAM)
                           (write-string (AREF ENGLISH-LARGE TRIAD) STREAM)
                           T)))))))))
...
(DEFUN ROMAN-CHAR (I X)
  (princ (CHAR "IVXLCDM" (+ I X X)) *STANDARD-OUTPUT*))


;;; Funny bases
(DEFUN (:PROPERTY :ENGLISH :PRINC-FUNCTION) (X STREAM)
  (FORMAT STREAM "~R" (- X)))

(DEFUN (:PROPERTY :ROMAN :PRINC-FUNCTION) (X STREAM)
  (FORMAT STREAM "~@R" (- X)))

(DEFUN (:PROPERTY :ROMAN-OLD :PRINC-FUNCTION) (X STREAM)
  (FORMAT STREAM "~:@R" (- X)))

...
;    (PRINT-FLONUM ARG STANDARD-OUTPUT NIL (TYPEP ARG 'SHORT-FLOAT)
;                     (CAR PARAMS) NIL)))

      ...
      RETRY
         (MULTIPLE-VALUE-BIND (ARG EXPONENT)
             (if (zerop original-arg)
                 ...
                 #|(SCALE-FLONUM (IF NEGATIVE ORIGINAL-ARG (- ORIGINAL-ARG)))|#
                 (values original-arg 1))
           ...
           (LET ((WIDTH-AFTER-SIGN-AND-EXPONENT
             ...
             (MULTIPLE-VALUE-BIND (BUFFER DECIMAL-PLACE)
               ...
               (LET ((EXTRA-ZERO (AND (≤ SCALE 0)
                 ...
                 (WHEN WIDTH
                   ...
                   (DOTIMES (I (- WIDTH-AFTER-SIGN-AND-EXPONENT (LENGTH BUFFER)
                                  (IF EXTRA-ZERO 1 0)))
                     (princ (OR PADCHAR #\SPACE) *STANDARD-OUTPUT*)))
                 (COND (NEGATIVE (princ #\- *STANDARD-OUTPUT*))
                       (*ATSIGN-FLAG* (princ #\+ *STANDARD-OUTPUT*)))
                 (WHEN EXTRA-ZERO
                   (princ #\0 *STANDARD-OUTPUT*)))
               (WHEN (MINUSP SCALE)
                 (princ (PTTBL-DECIMAL-POINT *READTABLE*) *STANDARD-OUTPUT*)
                 (DOTIMES (I (- SCALE))
                   (princ #\0 *STANDARD-OUTPUT*))
               ...
               (DOTIMES (I (1- (LENGTH BUFFER)))
                 (WHEN (= I SCALE)
                   (princ (PTTBL-DECIMAL-POINT *READTABLE*) *STANDARD-OUTPUT*))
                 (princ (CHAR BUFFER (IF (≥ I DECIMAL-PLACE) (1+ I) I))
                       *STANDARD-OUTPUT*))))
      ...
      (BLOCK NIL
        (LET* ((WIDTH (CAR PARAMS))
          ...
          (MULTIPLE-VALUE-BIND (NIL EXPONENT)
              (if (zerop arg)
                  (values arg 0)
                (SCALE-FLONUM (IF NEGATIVE ARG (- ARG))))
...
(DEFUN FORMAT-CTL-ASCII (ARG PARAMS &OPTIONAL PRIN1P)
  (LET ((EDGE (CAR PARAMS))
    ...
    (COND ((NULL PADCHAR)
           (SETQ PADCHAR #\SPACE))
          ((CHARACTERP PADCHAR))
          ((FIXNUMP PADCHAR) (SETQ PADCHAR (code-CHAR PADCHAR)))
          (T (SETQ PADCHAR (CHAR (STRING PADCHAR) 0))))
    (COND (*ATSIGN-FLAG*)                               ;~@5nA right justifies
          ((AND *COLON-FLAG* (NULL ARG)) (write-string "()" *STANDARD-OUTPUT*))
          (PRIN1P (PRIN1 ARG))
          ((STRINGP ARG) (write-string ARG *STANDARD-OUTPUT*))
          (T (PRINC ARG)))
    (WHEN EDGE
          ...
          ((NOT *COLON-FLAG*)
           ...
           (UNLESS (ZEROP BITS)
             ...
             (IF (≤ (CHAR-CODE #\a) (SETQ LOWER-CASE (CHAR-CODE ARG)) (CHAR-CODE #\z))
                 (write-string "sh-" *STANDARD-OUTPUT*)
               (SETQ LOWER-CASE NIL)))
           (COND (CHNAME
                  (SETQ CHNAME (SYMBOL-NAME CHNAME))
                  ;; are we CONSING yet?
                  (princ (CHAR-UPCASE (CHAR CHNAME 0)) *STANDARD-OUTPUT*)
                  (DO ((LEN (LENGTH CHNAME))
                       (I 1 (1+ I)))
                      ((= I LEN))
                    (princ (CHAR-DOWNCASE (CHAR CHNAME I)) *STANDARD-OUTPUT*)))
                 (T (IF *ATSIGN-FLAG*
                        (IF (CHARACTER-NEEDS-QUOTING-P (CHAR-CODE ARG))
                            (princ (PTTBL-SLASH *READTABLE*) *STANDARD-OUTPUT*))
                      (IF LOWER-CASE (SETQ ARG (CHAR-UPCASE (code-CHAR LOWER-CASE)))))
                    (princ (CHAR-CODE ARG) *STANDARD-OUTPUT*))))
          (T
           (PRINT-BITS BITS)
           (SETQ ARG (CODE-CHAR (CHAR-CODE ARG)))
           (COND ((SETQ CHNAME (FORMAT-GET-CHARACTER-NAME ARG))
                 ...
                 ((AND *ATSIGN-FLAG* (CHAR< ARG #\SPACE) (CHAR/= ARG #\Esc))
                  (princ ARG *STANDARD-OUTPUT*)
                  (FORMAT-PRINT-TOP-CHARACTER ARG))
                 ((AND (LOWER-CASE-P ARG)
                       (NOT (ZEROP BITS)))
                  (write-string "Shift-" *STANDARD-OUTPUT*)
                  (princ (CHAR-UPCASE ARG) *STANDARD-OUTPUT*))
                 (T (princ ARG *STANDARD-OUTPUT*)))))))

(DEFUN FORMAT-GET-CHARACTER-NAME (CHAR)
  (IF (FIXNUMP CHAR) (SETQ CHAR (code-char CHAR)))
  (UNLESS (AND (GRAPHIC-CHAR-P CHAR)
               (CHAR/= CHAR #\SPACE)
               (CHAR/= CHAR #\Esc))
    (DO ((L XR-SPECIAL-CHARACTER-NAMES (CDR L)))
        ((NULL L) NIL)
;character lossage
      (WHEN (CHAR= (code-char (CDAR L)) CHAR)
        (RETURN (CAAR L))))))
...
(DEFUN FORMAT-PRINT-TOP-CHARACTER (CHAR &AUX NAME CHNAME)
  ...
  (LET ((CODE (CHAR-INT CHAR)))
    (COND ((SETQ CHNAME (DOTIMES (I #o200)
                          (WHEN (= CODE (AREF KBD-NEW-TABLE 2 I))
                            (RETURN (code-char (AREF KBD-NEW-TABLE 1 I))))))
           (SETQ NAME " (Top-"))
          ((SETQ CHNAME (DOTIMES (I #o200)
                          (WHEN (= CODE (AREF KBD-NEW-TABLE 3 I))
                            (RETURN (code-char (AREF KBD-NEW-TABLE 0 I))))
                          (WHEN (= CODE (AREF KBD-NEW-TABLE 4 I))
                            (RETURN (code-char (AREF KBD-NEW-TABLE 1 I))))))
...
(DEFFORMAT % (:NO-ARG) (PARAMS &AUX (COUNT (OR (CAR PARAMS) 1)))
  (DOTIMES (I COUNT) (princ #\NEWLINE *STANDARD-OUTPUT*)))
...
(defun format-parse-clauses (closechar semip)
  (let ((start (+ 3 *ctl-index*))
        (clauses (make-array 30. :fill-pointer 0))
        (stack (make-array 10. :fill-pointer 0))
        i j tem *atsign-flag* *colon-flag* command)
    (setf (fill-pointer clauses) 0 (fill-pointer stack) 0)
    (setq i *ctl-index*)
    (do-forever
      (unless (setq *ctl-index* (zl::%string-search-char #\~ *ctl-string* *ctl-index* *ctl-length*))
        (ferror ;; Yow!
...

#|(defprop case-convert-stream t :io-stream-p)|#
...
(defformat \( (:multi-arg) (args params)
  (let ((clauses (format-parse-clauses '\) nil))
        ...
        (*case-converted-stream*
          (if (boundp *case-convert*)
              *standard-output* ;; *case-converted-stream*
              *standard-output*))
        #|(*standard-output* 'case-convert-stream)|#)
    (declare (special *case-converted-stream* *prev-char*))
    (unwind-protect
        (destructuring-bind (head . tail)
                            args
          (format-ctl-string (cons (upcase head) tail) ;TODO
                             (aref clauses 0)))
...
(defformat ∧ (:one-arg) (thing ignore)
  (let ((*downcase-flag* *atsign-flag*)
    ...
    (princ (char-upcase thing))))

#|(defprop upcase-stream t :io-stream-p)|#
...
(defformat → (:multi-arg) (args params)
  (let ((clauses (format-parse-clauses '← nil))
    ...
    (unwind-protect (format-ctl-string args (aref clauses 0))
      (format-reclaim-clauses clauses))))
(defformat ← (:no-arg) (ignore)
  (format-error "Stray ~~← in ~S control string" 'format))

...
(DEFUN PRINT-LIST (DESTINATION ELEMENT-FORMAT-STRING LIST
  ...
  (LET ((FSTRING (FORMAT NIL "~~~A{~~<~~%~A~~~D:;~A~~>~~^~A~~}"
    ...
    (cl:FORMAT DESTINATION FSTRING LIST))) ;!!!
...
(DEFFORMAT lambda.time.internal::time (:ONE-ARG) (UT IGNORE)
...
(DEFUN SCIENTIFIC-NUMBER (X)
  ...
  (COND ((ZEROP X)
         (VALUES 0 0 ""))
        ('ELSE
         (LET ((POW (* 3 (FLOOR (LOG (ABS X) 10) 3))))
           (VALUES (* X (EXPT 10.0 (- POW)))
                   POW
                   (CADR (ASSOC POW *SCIENTIFIC-NUMBER-POWER-TABLE* :test #'eq)))))))
